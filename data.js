...
const MACHINES = [
  ...
  // ========== NEUE SCHLEPPER ==========
  {id:'mf_8s',name:'Massey Ferguson 8S.305',icon:'🚜',price:55,
   typ:'Standardschlepper',hersteller:'Massey Ferguson (Frankreich)',ps:305,baujahr:'seit 2020',
   beschreibung:'Roter Schlepper mit moderner Kabine und stufenlosem Getriebe Dyna-VT.',
   einsatz:'Mittlere und große Höfe. Pflug, Säen, Heuernte, Anhänger.',
   funfact:'Massey Ferguson Schlepper wurden über 60 Jahre in Beauvais (FR) gebaut – über 1 Million Stück.'},
  {id:'nh_t7',name:'New Holland T7.270',icon:'🚜',price:50,
   typ:'Standardschlepper',hersteller:'New Holland (Italien/USA)',ps:270,baujahr:'seit 2019',
   beschreibung:'Blau-gelber Schlepper. Auto Command stufenloses Getriebe. Sehr komfortable Kabine.',
   einsatz:'Heuernte, Pflug, Frontlader, Stallarbeit.',
   funfact:'New Holland gehört zu CNH Industrial – wie Case IH und IVECO.'},
  {id:'case_magnum',name:'Case IH Magnum 250',icon:'🚜',price:55,
   typ:'Großschlepper',hersteller:'Case IH (USA)',ps:250,baujahr:'seit 2017',
   beschreibung:'Roter Schlepper mit kraftvollem Motor. Magnum-Reihe ist legendär in Amerika.',
   einsatz:'Pflug, Grubber, schwere Anhänger, Lohnunternehmer.',
   funfact:'Case IH baut seit 1842 Landmaschinen – eine der ältesten Marken der Welt.'},
  {id:'deutz_9',name:'Deutz-Fahr 9 Series',icon:'🚜',img:'img/machines/deutz_9.jpg',price:50,
   typ:'Großschlepper',hersteller:'Deutz-Fahr (Lauingen, DE)',ps:340,baujahr:'seit 2014',
   beschreibung:'Grüner deutscher Schlepper mit luftgekühltem Diesel-Motor. Sehr robust.',
   einsatz:'Universaler Hofschlepper bis Lohnarbeit.',
   funfact:'Deutz war 1864 die erste Firma die Verbrennungsmotoren in Serie baute (Otto Motor!).'},
  {id:'valtra',name:'Valtra T-Series',icon:'🚜',price:45,
   typ:'Premium-Schlepper',hersteller:'Valtra (Finnland, AGCO)',ps:235,baujahr:'seit 2014',
   beschreibung:'Finnischer Schlepper mit umkehrbarer Fahrtrichtung. Kabine kann sich drehen!',
   einsatz:'Forst, Kommunalarbeit, Hof. Unbegrenzt anpassbar.',
   funfact:'Valtra-Schlepper kann man auf Wunsch in jeder Farbe lackiert bekommen – sogar Pink.'},
  {id:'same_frutteto',name:'Same Frutteto',icon:'🚜',price:30,
   typ:'Schmalspur-Schlepper',hersteller:'Same (Italien, SDF)',ps:80,baujahr:'seit 2018',
   beschreibung:'Kleiner schmaler Schlepper für Wein- und Obstgärten. Passt zwischen Reben.',
   einsatz:'Weinbau, Obstanlagen, Hopfen. Unter 1,40 m Breite.',
   funfact:'Same gehört seit 1995 zu SDF – mit Deutz-Fahr und Lamborghini Trattori.'},

  // ========== HÄCKSLER ==========
  {id:'claas_jaguar',name:'Claas Jaguar 990',icon:'🌽',price:140,
   typ:'Selbstfahrender Häcksler',hersteller:'Claas (Harsewinkel, DE)',ps:925,baujahr:'seit 2020',
   beschreibung:'Großer Häcksler mit V12-Motor. Häckselt Mais und Gras für Silage.',
   einsatz:'Maisernte, Grasernte. Hauptkonkurrent zum Krone Big X.',
   funfact:'Claas baut Jaguar seit 1973 – mehr als 50.000 Stück verkauft weltweit.'},
  {id:'fendt_katana',name:'Fendt Katana 650',icon:'🌽',price:135,
   typ:'Selbstfahrender Häcksler',hersteller:'Fendt (Marktoberdorf, DE)',ps:653,baujahr:'seit 2012',
   beschreibung:'Häcksler von Fendt mit Mercedes-Motor. Sehr leise im Vergleich zur Konkurrenz.',
   einsatz:'Mais- und Grasernte für Silage und Biogas.',
   funfact:'Katana heißt Schwert auf japanisch – wegen der scharfen Häckseltrommel.'},

  // ========== BODENBEARBEITUNG ==========
  {id:'pflug',name:'Drehpflug 5-Schar',icon:'⛏️',price:25,
   typ:'Pflug-Anbaugerät',hersteller:'Lemken / Kverneland / Krone',ps:0,baujahr:'modern',
   beschreibung:'Wendet die Erde um – die obere Schicht kommt nach unten. 5 Scharen pflügen 5 Furchen gleichzeitig.',
   einsatz:'Im Herbst nach der Ernte. Lockert den Boden, vergräbt Pflanzenreste.',
   funfact:'Der Drehpflug spart Zeit: nach jeder Bahn wird der Pflug gedreht und in die andere Richtung weitergepflügt.'},
  {id:'kreiselegge',name:'Kreiselegge',icon:'🛞',price:20,
   typ:'Bodenbearbeitungs-Anbaugerät',hersteller:'Kuhn / Maschio / Lemken',ps:0,baujahr:'modern',
   beschreibung:'Mehrere senkrechte Wellen mit Zinken drehen sich. Krümeln den Boden fein für die Saat.',
   einsatz:'Saatbett-Vorbereitung. Oft mit Sämaschine kombiniert (Kompaktion).',
   funfact:'Eine Kreiselegge braucht viel Kraft – ein 12 m breites Gerät erfordert 400+ PS.'},
  {id:'grubber',name:'Grubber',icon:'🌿',price:18,
   typ:'Bodenlockerungs-Anbaugerät',hersteller:'Horsch / Köckerling / Lemken',ps:0,baujahr:'modern',
   beschreibung:'Mit Flügel-Scharen lockert den Boden tief – ohne ihn zu wenden wie der Pflug.',
   einsatz:'Stoppelbearbeitung nach der Ernte. Schneller als der Pflug.',
   funfact:'Pflugloser Anbau (mit Grubber statt Pflug) schont Bodenleben und Regenwürmer.'},

  // ========== AUSSAAT ==========
  {id:'saemaschine',name:'Sämaschine Cayena',icon:'🌱',price:30,
   typ:'pneumatische Sämaschine',hersteller:'Amazone (Hasbergen, DE)',ps:0,baujahr:'seit 2010',
   beschreibung:'Bläst Saatgut durch Schläuche zu den Furchen. 6 Meter Arbeitsbreite.',
   einsatz:'Direktsaat von Getreide, Raps, Senf. Ohne vorheriges Pflügen möglich.',
   funfact:'Eine moderne Sämaschine setzt jedes Korn auf den Millimeter genau.'},

  // ========== PFLEGE / DÜNGUNG ==========
  {id:'duengerstreuer',name:'Düngerstreuer ZA-V',icon:'💨',price:18,
   typ:'Düngerstreuer-Anbaugerät',hersteller:'Amazone (Hasbergen, DE)',ps:0,baujahr:'modern',
   beschreibung:'Wirft mit zwei rotierenden Wurfscheiben Düngerkörner gleichmäßig aufs Feld.',
   einsatz:'Mineraldünger streuen. Bis 50 Meter Wurfweite!',
   funfact:'GPS-Steuerung sorgt dafür dass jeder Quadratmeter genau die richtige Menge bekommt.'},
  {id:'amazone_spritze',name:'Pflanzenschutzspritze UG 2200',icon:'💧',price:35,
   typ:'Anhänge-Spritze',hersteller:'Amazone (Hasbergen, DE)',ps:0,baujahr:'modern',
   beschreibung:'Verteilt Pflanzenschutzmittel mit 24-36 m breiten Düsen-Gestängen.',
   einsatz:'Unkraut, Pilze und Schädlinge bekämpfen.',
   funfact:'Moderne Spritzen erkennen Unkraut per Kamera und sprühen NUR dort – spart 80% Mittel.'},
  {id:'guellefass',name:'Güllefass 18.000 L',icon:'💩',price:40,
   typ:'Güllewagen',hersteller:'Joskin / Zunhammer / Briri',ps:0,baujahr:'modern',
   beschreibung:'Riesiger Tank zum Ausbringen von Gülle. Mit Schleppschlauch direkt zum Boden.',
   einsatz:'Frühjahr und Sommer. Gülle ist wertvoller Naturdünger.',
   funfact:'18.000 Liter Gülle wiegen 18 Tonnen – schwerer als ein LKW!'},

  // ========== HEUERNTE ==========
  {id:'maehwerk',name:'Trommelmähwerk',icon:'✂️',price:18,
   typ:'Mähwerk-Anbaugerät',hersteller:'Krone / Pöttinger / Claas',ps:0,baujahr:'modern',
   beschreibung:'Schneidet das Gras mit rotierenden Trommeln oder Scheiben mit Klingen.',
   einsatz:'Heuernte. Erste Arbeit – Gras wird abgemäht.',
   funfact:'Profis mähen mit 15 Meter breit – Front + Heck + zwei seitlich am Schlepper.'},
  {id:'heuwender',name:'Kreiselheuer',icon:'🍃',price:15,
   typ:'Heuwender-Anbaugerät',hersteller:'Pöttinger / Krone / Kuhn',ps:0,baujahr:'modern',
   beschreibung:'Wirft das geschnittene Gras hoch – damit es schneller trocknet.',
   einsatz:'Tag 2 der Heuernte. Gras wird gewendet zum Trocknen.',
   funfact:'Heu wird nur trocken eingefahren – sonst schimmelt es im Stall (Brandgefahr!).'},
  {id:'schwader',name:'Schwader',icon:'🌾',price:18,
   typ:'Schwader-Anbaugerät',hersteller:'Pöttinger / Krone / Claas',ps:0,baujahr:'modern',
   beschreibung:'Schiebt das trockene Gras zu langen Reihen (Schwaden) zusammen.',
   einsatz:'Tag 3 der Heuernte. Bereitet alles für die Ballenpresse vor.',
   funfact:'Ein Schwader hat manchmal 4 Kreisel, schwadet 15 Meter breit auf einmal.'},

  // ========== PRESSEN ==========
  {id:'bigpack',name:'Krone BiG Pack 1270',icon:'📦',img:'img/machines/bigpack.jpg',price:50,
   typ:'Quaderballenpresse',hersteller:'Krone (Spelle, DE)',ps:0,baujahr:'seit 2014',
   beschreibung:'Presst Heu/Stroh in riesige Quaderballen 1,20 x 0,70 x 2,80 m.',
   einsatz:'Großbetriebe. Quaderballen lassen sich besser stapeln und transportieren.',
   funfact:'Ein BiG Pack-Ballen wiegt 500-700 kg – soviel wie ein Klein-Pferd!'},
  {id:'round_baler',name:'Rundballenpresse',icon:'⭕',price:40,
   typ:'Rundballenpresse',hersteller:'Krone / Vicon / John Deere',ps:0,baujahr:'modern',
   beschreibung:'Wickelt Heu/Stroh zu runden Ballen mit 1,20-1,80 m Durchmesser.',
   einsatz:'Klein- und Mittelhöfe. Rundballen kann auch ein 30-PS-Schlepper transportieren.',
   funfact:'Rundballen können auf der Wiese liegen bleiben – Regen fließt einfach ab.'},

  // ========== ERNTE-SPEZIAL ==========
  {id:'kartoffelroder',name:'Kartoffelroder',icon:'🥔',price:60,
   typ:'Kartoffel-Erntemaschine',hersteller:'Grimme (Damme, DE) / Ropa',ps:0,baujahr:'modern',
   beschreibung:'Gräbt die Kartoffeln aus, trennt sie von Erde und Steinen, lädt sie auf den Anhänger.',
   einsatz:'September-Oktober. Erntet 80 Tonnen Kartoffeln pro Stunde.',
   funfact:'Grimme aus Niedersachsen baut die meisten Kartoffelroder weltweit – 70% Marktanteil.'},

  // ========== TRANSPORT / HOFGERÄTE ==========
  {id:'jcb_telehandler',name:'JCB Teleskoplader 542-70',icon:'🏗️',price:70,
   typ:'Teleskoplader',hersteller:'JCB (Großbritannien)',ps:145,baujahr:'modern',
   beschreibung:'Wie ein Frontlader nur mit ausziehbarem Arm bis 7 Meter hoch. Mega vielseitig.',
   einsatz:'Stallarbeit, Strohballen stapeln, Silo befüllen, Bauarbeiten.',
   funfact:'JCB-Teleskoplader können bis zu 4 Tonnen heben – mehr als ein Auto.'},
  {id:'forwarder',name:'JD Forwarder 1110G',icon:'🌲',price:80,
   typ:'Forst-Tragschlepper',hersteller:'John Deere',ps:204,baujahr:'modern',
   beschreibung:'Forst-Maschine die gefällte Baumstämme aus dem Wald trägt. Mit Greifarm.',
   einsatz:'Holzernte. Sammelt Stämme die der Harvester gefällt hat.',
   funfact:'Der Forwarder fährt auf 6 oder 8 Rädern - manchmal mit Bändern damit er nicht im Boden versinkt.'}
];
...
const CHARS = [
  {id:'mario',name:'Mario',icon:'🍄',img:'img/chars/mario.png',color:'#e53935',price:5,move:'jump',sound:'wahoo',
   ...
   welt:'Super Mario',type:'Klempner-Held',
   desc:'Marios bester Freund ist Luigi. Er rettet immer Prinzessin Peach vor Bowser. Springt auf Pilze und sammelt Münzen.'},
  {id:'luigi',name:'Luigi',icon:'🟢',img:'img/chars/luigi.png',color:'#43a047',price:10,move:'tallJump',sound:'oh-yeah',
   ...
   welt:'Super Mario',type:'Marios Bruder',
   desc:'Marios grüner Bruder. Etwas ängstlich aber sehr loyal. Springt höher als Mario.'},
  {id:'sonic',name:'Sonic',icon:'💨',img:'img/chars/sonic.png',color:'#1976d2',price:20,move:'dash',sound:'whoosh',
   ...
   welt:'Sonic the Hedgehog',type:'Blauer Igel',
   desc:'Der schnellste Igel der Welt. Sammelt goldene Ringe und kämpft gegen Dr. Eggman. Liebt Chili Dogs!'},
  {id:'tails',name:'Tails',icon:'🦊',img:'img/chars/tails.png',color:'#fb8c00',price:30,move:'fly',sound:'spin',
   ...
   welt:'Sonic the Hedgehog',type:'Fliegender Fuchs',
   desc:'Sonics bester Freund. Hat zwei Schwänze die er als Helikopter benutzt. Sehr klug, baut Maschinen.'},
  {id:'yoshi',name:'Yoshi',icon:'🦖',img:'img/chars/yoshi.png',color:'#7cb342',price:40,move:'tongue',sound:'yoshi',
   ...
   welt:'Super Mario',type:'Grüner Dino',
   desc:'Marios treuer Reit-Dino. Frisst Feinde mit seiner langen Zunge und legt Eier zum Werfen.'},
  {id:'knuckles',name:'Knuckles',icon:'👊',img:'img/chars/knuckles.png',color:'#d84315',price:60,move:'punch',sound:'pow',
   ...
   welt:'Sonic the Hedgehog',type:'Roter Echidna',
   desc:'Sehr stark und kann mit seinen Stachel-Knöcheln durch Felsen schlagen. Hütet den Master-Smaragd.'},
  {id:'peach',name:'Peach',icon:'👑',img:'img/chars/peach.png',color:'#ec407a',price:80,move:'float',sound:'shiny',
   ...
   welt:'Super Mario',type:'Prinzessin',
   desc:'Prinzessin des Pilz-Königreichs. Bowser entführt sie ständig - aber sie kann auch selbst kämpfen!'},
  {id:'shadow',name:'Shadow',icon:'⚡',img:'img/chars/shadow.png',color:'#212121',price:100,move:'teleport',sound:'zap',
   ...
   welt:'Sonic the Hedgehog',type:'Schwarzer Igel',
   desc:'Wurde im Labor erschaffen. Genauso schnell wie Sonic, aber finsterer. Nutzt Chaos Control zum Teleportieren.'},
  {id:'bowser',name:'Bowser',icon:'🐢',img:'img/chars/bowser.png',color:'#6d4c41',price:130,move:'roar',sound:'roar',
   ...
   welt:'Super Mario',type:'Koopa-König',
   desc:'Marios größter Feind. König der Koopas, spuckt Feuer und entführt Peach. Hat 7 Söhne (Koopalinge).'},
  {id:'eggman',name:'Dr. Eggman',icon:'🥚',img:'img/chars/eggman.png',color:'#f57f17',price:160,move:'spin',sound:'evil-laugh',
   ...
   welt:'Sonic the Hedgehog',type:'Verrückter Wissenschaftler',
   desc:'Sonics Erzfeind. Riesiger Schnurrbart, IQ 300. Baut böse Roboter um die Welt zu erobern.'},
  {id:'rosalina',name:'Rosalina',icon:'⭐',img:'img/chars/rosalina.png',color:'#7b1fa2',price:200,move:'spin-star',sound:'sparkle',
   ...
   welt:'Super Mario',type:'Stern-Prinzessin',
   desc:'Lebt im Weltraum auf einem Stern-Schiff mit ihren Lumas. Mutter aller Sterne, sehr weise.'},
  {id:'superstar',name:'Superstar',icon:'🌟',color:'#fbc02d',price:250,move:'rainbow',sound:'fanfare',
   ...
   welt:'Super Mario',type:'Power-Up',
   desc:'Macht Mario unverwundbar! Wenn du den Stern berührst funkelt Mario in Regenbogenfarben.'},
  // ===== Ninjago-Welt =====
  {id:'kai',name:'Kai',icon:'🔥',img:'img/chars/ninja_group.jpg',color:'#e53935',price:35,move:'roar',sound:'fire-whoosh',
   sayings:['Ninja-Go!','Feuer frei!','Kai ist heiss!','Lass mich ran!'],
   welt:'Lego Ninjago',type:'Roter Feuer-Ninja',
   desc:'Hitziger Schmied-Ninja. Sein Element ist Feuer. Beste Freunde mit den anderen 5 Ninja.'},
  {id:'jay',name:'Jay',icon:'⚡',img:'img/chars/ninja_group.jpg',color:'#1976d2',price:45,move:'teleport',sound:'thunder',
   sayings:['BZZZT!','Blitzschnell!','Jay-Style!','Voll geladen!'],
   welt:'Lego Ninjago',type:'Blauer Blitz-Ninja',
   desc:'Erfinder und Witzbold. Sein Element ist Blitz. Liebt Nya und macht ständig Witze.'},
  {id:'cole',name:'Cole',icon:'🪨',img:'img/chars/ninja_group.jpg',color:'#5d4037',price:55,move:'punch',sound:'earth-stomp',
   sayings:['BOOM!','Stein an Stein!','Cole rockt!','Erschuetterung!'],
   welt:'Lego Ninjago',type:'Schwarzer Erd-Ninja',
   desc:'Anführer der Ninja. Stärkster im Team, sein Element ist Erde. Liebt Kuchen über alles.'},
  {id:'zane',name:'Zane',icon:'❄️',img:'img/chars/ninja_group.jpg',color:'#81d4fa',price:70,move:'float',sound:'ice-crystal',
   sayings:['Eiskalt.','Zane berechnet alles.','Frost!','Mein System ist optimal.'],
   welt:'Lego Ninjago',type:'Weißer Eis-Ninja',
   desc:'Roboter-Ninja (Nindroid)! Sehr klug und ruhig. Sein Element ist Eis. Versteht keine Witze.'},
  {id:'lloyd',name:'Lloyd Garmadon',icon:'🟢',img:'img/chars/lloyd.webp',color:'#43a047',price:90,move:'spin-star',sound:'energy-burst',
   sayings:['Gruener Ninja!','Lloyd kommt!','Energie raus!','Ich bin der Auserwaehlte!'],
   welt:'Lego Ninjago',type:'Grüner Energie-Ninja',
   desc:'Der Auserwählte! Sohn von Lord Garmadon. Sein Element ist Energie - vereint alle Elemente.'},
  {id:'nya',name:'Nya',icon:'🌊',img:'img/chars/ninja_group.jpg',color:'#0277bd',price:110,move:'float',sound:'water-splash',
   sayings:['Wasser marsch!','Nya weiss wie es geht!','Spritzer!','Komm rein, das Wasser ist fein!'],
   welt:'Lego Ninjago',type:'Wasser-Ninja',
   desc:'Kais kleine Schwester und einzige Ninja-Frau im Team. Ihr Element ist Wasser, fährt Drachen-Mech.'},
  {id:'wu',name:'Master Wu',icon:'🍵',img:'img/chars/wu.jpg',color:'#fff9c4',price:180,move:'spin',sound:'gong',
   ...
   welt:'Lego Ninjago',type:'Lehrer',
   desc:'Uralter Sensei der Ninja. Trinkt grünen Tee, kennt alle Geheimnisse von Spinjitzu. Bruder von Garmadon.'},
...
const SUBJECTS_TREE = {
  liam: {
    ...
    schlau: {label:'🦊 Straßenschläue', emoji:'🦊', subs: [
      {id:'alltag', label:'🦊 Schlaufuchs-Fragen', desc:'Geld, Werbung, Notfall, Alltag', special:'schlau'}
    ]},
...
// ============================================================
// STRASSENSCHLÄUE 🦊 – unternehmerisches + logisches Denken,
// Erste Hilfe, Alltag. Kern: die WARUM-Erklärung nach der Antwort.
// ============================================================
const SCHLAU_RAIK = [
  {q:'Ein Eis kostet 2 €. Das 6er-Pack kostet 6 €. Ihr seid 6 Kinder. Was ist schlau?',options:['Jeder kauft einzeln','Pack kaufen und teilen','Gar kein Eis','Jeder kauft 2'],correct:1,img:'🍦',why:'Einzeln wären das 12 €! Das Pack kostet nur 6 € – die Hälfte. Teilen spart Geld und alle sind glücklich.'},
  {q:'Du hast 5 €. Das Spielzeug kostet 7 €. Was machst du?',options:['Geld leihen','Sparen bis es reicht','5 € für Süßes ausgeben','Quengeln'],correct:1,img:'🧸',why:'Wer ein bisschen warten kann, bekommt, was er WIRKLICH will. Das ist ein echter Superhelden-Trick beim Geld!'},
  {q:'In der Werbung sieht der Burger riesig und glänzend aus. Wie ist er in echt?',options:['Genauso','Meist kleiner und platter','Noch größer','Aus Gold'],correct:1,img:'🍔',why:'Werbung ist wie ein geschminktes Foto – sie will verkaufen, darum sieht alles toller aus als in echt.'},
  {q:'Dein Freund will deine Lieblingskarte gegen einen Sticker tauschen. Was ist schlau?',options:['Sofort tauschen','Erst überlegen: Was ist MIR wichtiger?','Karte verstecken','Streiten'],correct:1,img:'🃏',why:'Beim Tauschen zählt, was DIR wichtiger ist – nicht, was der andere toll findet. Einmal drüber schlafen ist nie verkehrt.'},
  {q:'Was ziehst du ZUERST an?',options:['Schuhe, dann Socken','Socken, dann Schuhe','Beides gleichzeitig','Nur Schuhe'],correct:1,img:'🧦',why:'Die richtige Reihenfolge macht vieles leichter – beim Anziehen, beim Bauen, beim Rechnen. Erst denken, dann machen!'},
  {q:'Kleine Tüte Gummibärchen: 1 €. Große Tüte mit doppelt so viel: 1,50 €. Was ist der bessere Deal?',options:['Kleine Tüte','Große Tüte','Beide gleich','Keine'],correct:1,img:'🍬',why:'Doppelt so viele Gummibärchen für nur 50 Cent mehr! Vergleichen lohnt sich fast immer.'},
  {q:'Du findest 5 € auf dem Schulhof. Was tust du?',options:['Schnell einstecken','Der Lehrerin geben','Verstecken','Süßes kaufen'],correct:1,img:'💶',why:'Irgendein Kind sucht bestimmt traurig sein Geld. Ehrlich sein fühlt sich am Ende am besten an – versprochen.'},
  {q:'Ein Fremder sagt: „Komm mit, ich zeige dir Welpen im Auto!" Was tust du?',options:['Mitgehen','Laut NEIN sagen und sofort zu einem Erwachsenen, dem du vertraust','Leise gucken gehen','Nur kurz mitgehen'],correct:1,img:'🚗',why:'Merk dir: Erwachsene bitten Kinder NIE um Hilfe. Du darfst laut NEIN sagen und weggehen – das ist mutig, nicht unhöflich!'},
  {q:'Welche Nummer rufst du bei einem Notfall an?',options:['110','112','999','123'],correct:1,img:'📞',why:'1-1-2 – die Nummer für Feuerwehr und Krankenwagen. Sie funktioniert von jedem Handy, sogar ohne Guthaben!'},
  {q:'Was sagst du am Notruf-Telefon?',options:['„Hilfe!" und auflegen','Wer ich bin, wo ich bin, was passiert ist','Einen Witz','Gar nichts'],correct:1,img:'☎️',why:'WER – WO – WAS. Und ganz wichtig: nicht auflegen, bis die netten Leute am Telefon alles wissen!'},
  {q:'Oma stolpert, ihr Knie blutet etwas. Was tust du?',options:['Weglaufen','Einen Erwachsenen holen und Oma trösten','Selbst Doktor spielen','Weiter fernsehen'],correct:1,img:'🩹',why:'Du bist ein echter Held, wenn du schnell Hilfe holst! Trösten kannst du sofort – verarzten machen die Großen.'},
  {q:'Du hast dich im Wald verlaufen. Was ist am schlausten?',options:['Weiterlaufen','Stehen bleiben und laut rufen','Verstecken','Beeren essen'],correct:1,img:'🌲',why:'Wer stehen bleibt, wird viel schneller gefunden! Die Sucher kommen zu dir – und dein Rufen zeigt ihnen den Weg.'},
  {q:'Gewitter! Du bist im Schwimmbad im Wasser. Was tust du?',options:['Sofort raus aus dem Wasser','Weiterplanschen','Untertauchen','Unter den großen Baum'],correct:0,img:'⛈️',why:'Blitze lieben Wasser und einzelne Bäume. Raus aus dem Wasser und rein ins Gebäude – dort bist du sicher.'},
  {q:'Es brennt in der Küche! Was tust du?',options:['Spielzeug holen','Sofort raus und draußen 112 rufen','Unterm Bett verstecken','Zugucken'],correct:1,img:'🔥',why:'RAUS ist deine einzige Aufgabe! Nichts holen, nicht verstecken. Draußen bist du sicher und rufst 112 – löschen machen die Profis.'},
  {q:'Du entdeckst eine Zecke an deinem Bein. Was tust du?',options:['Selbst rauspulen','Sofort einem Erwachsenen zeigen','Ignorieren','Draufhauen'],correct:1,img:'🕷️',why:'Zecken müssen vorsichtig und ganz raus – das können Erwachsene am besten. Du bist schlau, wenn du es gleich zeigst!'},
  {q:'Was solltest du auswendig wissen?',options:['Alle 251 Pokémon','Mamas oder Papas Handynummer','Alle Autofarben','Das Fernsehprogramm'],correct:1,img:'📱',why:'Wenn du mal Hilfe brauchst, ist die Nummer von Mama oder Papa dein Zauberschlüssel. Übe sie wie einen Reim!'},
  {q:'5 Münzen ausgeben ODER eine Gießkanne kaufen und nächste Woche 10 Tomaten ernten. Was ist Investieren?',options:['Ausgeben für Süßes','Die Gießkanne kaufen','Münzen verstecken','Münzen verlieren'],correct:1,img:'🍅',why:'Investieren heißt: erst etwas Nützliches kaufen, das dir SPÄTER mehr bringt. Aus 5 Münzen werden 10 Tomaten!'},
  {q:'Du bekommst jede Woche 2 €. Was macht dich nächsten Monat reicher?',options:['Jede Woche 1 € ins Sparschwein','Alles sofort für Kaugummi','Alles verlieren','Nichts'],correct:0,img:'🐷',why:'Kleine Beträge werden groß, wenn man dranbleibt: Nach 4 Wochen hast du 4 € gespart UND trotzdem jede Woche Kaugummi gehabt!'},
  {q:'„Geld bekommt Babys, wenn es arbeitet." Was bedeutet das?',options:['Geldscheine heiraten','Gespartes Geld kann mehr Geld bringen (Zinsen)','Geld weint nachts','Nichts'],correct:1,img:'💰',why:'Wenn dein Geld auf der Bank „arbeitet", bekommst du ein bisschen dazu geschenkt – das nennt man Zinsen.'},
  {q:'Du hast 6 Kekse, ihr seid 3 Kinder. Wie teilst du fair?',options:['Ich esse alle','Jeder bekommt 2','Einer bekommt 6','Wegwerfen'],correct:1,img:'🍪',why:'6 geteilt durch 3 = 2 für jeden. Wer fair teilt, mit dem teilen die anderen beim nächsten Mal auch!'},
  {q:'Was kommt ZULETZT vor dem Schlafen?',options:['Süßes essen','Zähne putzen','Chips essen','Saft trinken'],correct:1,img:'🦷',why:'Erst naschen, DANN putzen – sonst feiern die Zuckermonster die ganze Nacht Party auf deinen Zähnen.'},
  {q:'Was passiert, wenn du deine Blume NIE gießt?',options:['Sie wächst besser','Sie vertrocknet','Sie wird blau','Sie singt'],correct:1,img:'🌻',why:'Manche Dinge brauchen dich regelmäßig: Blumen, Haustiere, Freundschaften. Dranbleiben ist eine Superkraft!'},
  {q:'Ein Spiel ist „GRATIS!", aber die Diamanten im Spiel kosten echtes Geld. Was ist der Trick?',options:['Alles ist wirklich gratis','Sie wollen, dass du später Geld ausgibst','Diamanten sind echt','Es gibt keinen Trick'],correct:1,img:'💎',why:'„Gratis" ist oft ein Köder – der Laden verdient erst, wenn du drin bist. Frag immer erst Mama oder Papa.'},
  {q:'Die Ampel ist grün, aber ein Auto kommt schnell angefahren. Was tust du?',options:['Sofort loslaufen','Warten und schauen, ob es wirklich hält','Augen zu und los','Winken'],correct:1,img:'🚦',why:'Grün heißt „du darfst" – aber deine Augen sind der beste Schutz. Erst gucken, dann gehen!'},
  {q:'Dein Freund ist traurig. Was machst du?',options:['Auslachen','Fragen, was los ist, und trösten','Weggehen','Petzen'],correct:1,img:'🤗',why:'Wer tröstet, ist ein wahrer Freund. Und wenn DU mal traurig bist, ist er für dich da.'},
  {q:'Du willst einen fremden Hund streicheln. Was zuerst?',options:['Einfach anfassen','Den Besitzer fragen','Am Schwanz ziehen','Anschreien'],correct:1,img:'🐕',why:'Auch der liebste Hund erschrickt bei fremden Händen. Erst fragen, dann langsam die Hand hinhalten – so machen es Hundeprofis.'},
  {q:'Warum liegen Süßigkeiten genau an der Kasse?',options:['Zufall','Damit du beim Warten quengelst und die Eltern kaufen','Weil es da kühl ist','Weil sie da frisch bleiben'],correct:1,img:'🍫',why:'Der Laden ist schlau – aber du bist schlauer! Sie legen Süßes dahin, wo alle warten müssen. Wer den Trick kennt, fällt nicht drauf rein.'},
  {q:'Du willst schaukeln, aber ein Kind ist dran. Was ist schlau?',options:['Runterschubsen','Fragen, ob ihr euch abwechselt','Laut weinen','Wegnehmen'],correct:1,img:'🛝',why:'Mit Fragen bekommst du fast immer, was du willst – und einen Spielkameraden dazu. Schubsen bringt nur Ärger.'},
  {q:'Der Topf auf dem Herd qualmt stark. Was tust du?',options:['Selbst anfassen','Sofort einen Erwachsenen rufen','Wasser reinschütten','Zugucken'],correct:1,img:'🍳',why:'Rufen ist Heldensache, Anfassen ist Erwachsenensache. Und Wasser in heißes Fett wäre sehr gefährlich – das wissen nicht mal alle Großen!'},
  {q:'1 € heute oder 2 € morgen – was nimmst du?',options:['1 € heute','2 € morgen','Gar nichts','Egal'],correct:1,img:'⏳',why:'Warten verdoppelt hier dein Geld! Geduld ist beim Geld eine echte Superkraft.'},
  {q:'Wann trägst du deinen Fahrradhelm?',options:['Nur bei langen Touren','Immer beim Fahren','Nie','Nur im Winter'],correct:1,img:'🚲',why:'Dein Kopf ist dein wertvollster Schatz – der Helm ist seine Ritterrüstung. Echte Rennfahrer tragen ihn auch immer!'},
  {q:'Jemand im Online-Spiel fragt nach deinem Namen und deiner Adresse. Was tust du?',options:['Alles sagen','Nichts verraten und es Mama oder Papa zeigen','Nur die Adresse','Nur die Schule'],correct:1,img:'🎮',why:'Im Internet weißt du nie, wer wirklich am anderen Ende sitzt. Deine Daten sind geheim wie ein Schatz.'},
  {q:'Im Laden liegt ein kaputtes Spielzeug für nur 1 €. Kaufen?',options:['Klar, ist ja billig','Nein – kaputt bringt auch für 1 € nichts','Zwei davon','Drei davon'],correct:1,img:'🤖',why:'Billig gekauft ist zweimal gekauft! Was kaputt ist, macht keine Freude – egal wie wenig es kostet.'},
  {q:'Du verstehst die Hausaufgabe nicht. Was ist schlau?',options:['Abschreiben','Lehrer oder Eltern fragen','Gar nicht machen','Raten'],correct:1,img:'📚',why:'Fragen ist nie peinlich – es ist das Schlauste überhaupt! Wer fragt, versteht. Wer abschreibt, kann es morgen immer noch nicht.'},
  {q:'Ein Kind wird auf dem Schulhof geärgert. Was tust du?',options:['Mitmachen','Laut „Stopp!" sagen und einen Erwachsenen holen','Nur zugucken','Es filmen'],correct:1,img:'🛑',why:'Dein „Stopp!" kann jemandem den ganzen Tag retten. Hilfe holen ist kein Petzen – es ist Heldensache.'}
];

const SCHLAU_LIAM = [
  {q:'Ernte-Deal: Heute 3 Münzen pro Sack. Lagern kostet 1 Münze pro Sack, im Winter gibt es 5 Münzen. Was lohnt sich?',options:['Sofort verkaufen','Lagern und im Winter verkaufen','Wegwerfen','Verschenken'],correct:1,img:'🌾',why:'Rechnen wir: 5 Münzen minus 1 Münze Lager = 4 Münzen. Das ist mehr als 3! Gute Bauern rechnen, BEVOR sie verkaufen.'},
  {q:'Dein Traktor ist kaputt. Reparatur: 200 €. Neuer Traktor: 20.000 €. Was prüfst du zuerst?',options:['Sofort neu kaufen','Ob die Reparatur reicht','Traktor stehen lassen','Fahrrad nehmen'],correct:1,img:'🚜',why:'Die Reparatur kostet nur ein Hundertstel! Erst reparieren prüfen, dann über Neukauf nachdenken – so machen es kluge Unternehmer.'},
  {q:'Dünger: großer Sack 25 kg für 20 €, kleiner Sack 5 kg für 6 €. Welcher ist PRO KILO günstiger?',options:['Der kleine','Der große','Beide gleich','Kommt auf die Farbe an'],correct:1,img:'🧮',why:'Großer Sack: 20 € geteilt durch 25 kg = 80 Cent pro Kilo. Kleiner: 6 € geteilt durch 5 kg = 1,20 € pro Kilo! Immer den Kilopreis vergleichen.'},
  {q:'Billige Schaufel für 5 € (bricht nach 1 Woche) oder gute für 15 € (hält 5 Jahre)?',options:['Die billige','Die gute','Beide','Keine'],correct:1,img:'🛠️',why:'„Billig gekauft = zweimal gekauft!" Drei kaputte Billig-Schaufeln kosten mehr als eine gute – und nerven dreimal.'},
  {q:'Du sparst auf einen großen Trecker. Was ist der beste Plan?',options:['Hoffen','Von jeder Ernte 10 % zur Seite legen','Alles ausgeben und warten','Auf Geldregen warten'],correct:1,img:'🏦',why:'Profi-Trick: immer einen festen Teil zur Seite legen, BEVOR du den Rest ausgibst. So wächst dein Trecker-Konto von ganz allein.'},
  {q:'„GRATIS Traktor-Poster – wenn du 3 Zeitschriften kaufst!" Was fragst du dich?',options:['Nichts, GRATIS!','Will ich die Zeitschriften überhaupt?','Wo ist die Schere?','Gibt es noch mehr Poster?'],correct:1,img:'🎁',why:'Der Köder ist das Poster, das Geschäft sind die Zeitschriften. „Gratis" ist fast nie gratis – rechne aus, was du WIRKLICH bezahlst.'},
  {q:'Flohmarkt: Der Verkäufer will 10 €, du hast nur 7 €. Was ist schlau?',options:['Traurig weggehen','Freundlich fragen: „Geht auch 7 €?"','Fordern: „Her damit!"','10 € leihen'],correct:1,img:'🤝',why:'Freundlich verhandeln kostet nichts und klappt erstaunlich oft – besonders am Ende des Flohmarkts. Freundlichkeit gewinnt.'},
  {q:'Heuballen: Händler A: 4 € pro Ballen mit Lieferung. Händler B: 3,50 € plus 20 € Lieferung. Du brauchst 10 Ballen.',options:['Händler A (40 €)','Händler B (55 €)','Beide gleich','Der mit dem netteren Hut'],correct:0,img:'🌾',why:'A: 10 mal 4 = 40 €. B: 10 mal 3,50 plus 20 Lieferung = 55 €! Der billigere Stückpreis kann durch Nebenkosten teurer werden – immer ALLES zusammenrechnen.'},
  {q:'Ein neuer Anhänger kostet 100 Münzen, aber du schaffst damit DOPPELT so viel Ernte pro Fahrt. Wann lohnt er sich?',options:['Nie','Wenn du oft fährst','Nur wenn er rot ist','Sofort wieder verkaufen'],correct:1,img:'🚜',why:'Das ist Investieren: erst bezahlen, dann bei JEDER Fahrt sparen. Wer oft fährt, hat die 100 Münzen schnell wieder drin – ab dann ist alles Gewinn.'},
  {q:'Du legst 100 € auf die Bank. Die Bank zahlt 3 % Zinsen im Jahr. Was passiert?',options:['Es bleiben 100 €','Nach einem Jahr sind es 103 €','Es werden 300 €','Die Bank behält alles'],correct:1,img:'💰',why:'Zinsen heißt: Dein Geld arbeitet für dich, während du schläfst. Und nächstes Jahr gibt es Zinsen auf 103 € – so wächst es immer schneller!'},
  {q:'Werbung: „DER STÄRKSTE TRAKTOR DER WELT!!!" Was prüft ein schlauer Käufer?',options:['Nichts, klingt super','Echte Zahlen: PS, Verbrauch, Preis – und Vergleich mit anderen','Die Größe der Buchstaben','Ob Feuerwerk im Video ist'],correct:1,img:'📊',why:'Je lauter die Werbung schreit, desto genauer hinschauen! Zahlen lügen nicht – Werbesprüche schon mal. Vergleichen ist deine Geheimwaffe.'},
  {q:'Notfall auf dem Hof! Du rufst 112 an. Was sagst du?',options:['„Kommt schnell!" und auflegen','WER ich bin, WO wir sind, WAS passiert ist','Nur den Hofnamen','Erst mal nichts'],correct:1,img:'📞',why:'WER – WO – WAS, und dranbleiben, bis alle Fragen beantwortet sind. Genau das macht dich im Notfall zum wichtigsten Helfer.'},
  {q:'Jemand hat sich an der Maschine verletzt und blutet stark. Was tust du?',options:['Selbst verarzten','Laut um Hilfe rufen, 112, bei der Person bleiben und reden','Wegrennen','Foto machen'],correct:1,img:'🚑',why:'Dein Job als Held: Hilfe holen, dableiben, beruhigen. Verarzten übernehmen die Profis – die sind schnell da, wenn DU gut erklärst, wo ihr seid.'},
  {q:'Nach dem Sturm liegt eine Stromleitung auf dem Feld. Was tust du?',options:['Wegräumen','GROSSEN Abstand halten und 112 rufen','Drüberspringen','Kurz anfassen zum Testen'],correct:1,img:'⚡',why:'Strom sieht man nicht und hört man nicht – er kann sogar durch den Boden kriechen. Mindestens 20 große Schritte Abstand und sofort melden!'},
  {q:'Gewitter, du bist draußen auf dem freien Feld. Was ist richtig?',options:['Unter den einzelnen großen Baum','In ein Gebäude oder Auto – notfalls hinhocken, Füße zusammen','Auf den Hügel klettern','Weiterarbeiten'],correct:1,img:'⛈️',why:'Autos und Gebäude sind sichere Blitz-Käfige. Einzelne Bäume und Hügel sind Blitz-Magnete. Hocken statt liegen – kleinste Angriffsfläche!'},
  {q:'Du hast dich im Wald verlaufen. Was ist der Profi-Trick?',options:['Immer weiterlaufen','An einer markanten Stelle bleiben und laut rufen','Einem Tier folgen','Auf einen Baum klettern'],correct:1,img:'🌲',why:'Suchtrupps finden ein wartendes Kind viel schneller als ein laufendes. Bleib an einem auffälligen Ort und mach auf dich aufmerksam.'},
  {q:'Feuer in der Scheune! Was ist DEINE Aufgabe?',options:['Tiere selbst retten','Sofort raus und 112 – die Feuerwehr rettet die Tiere','Wasser holen','Zugucken'],correct:1,img:'🔥',why:'Das ist schwer, aber wichtig: Deine Aufgabe ist RAUS und ANRUFEN. Die Feuerwehr hat Schutzanzüge und rettet Tiere viel besser – du hilfst am meisten, wenn du schnell meldest.'},
  {q:'Nach der Feldarbeit findest du eine Zecke an dir. Was tust du?',options:['Rauspulen und vergessen','Erwachsenem zeigen, entfernen lassen, Stelle beobachten','Ignorieren','Mit Öl beträufeln'],correct:1,img:'🕷️',why:'Nach dem Entfernen die Stelle im Blick behalten: Wird sie rot wie eine Zielscheibe, ab zum Arzt – früh erkannt ist es gut behandelbar.'},
  {q:'Jemand liegt bewusstlos NEBEN einem offenen Stromkasten. Was zuerst?',options:['Sofort hinfassen und wegziehen','NICHT anfassen – erst Strom weg oder Abstand, 112 rufen','Wasser drüberschütten','Wachrütteln'],correct:1,img:'⚠️',why:'Wichtigste Helfer-Regel: Erst schauen, ob es für DICH sicher ist! Sonst wirst du selbst zum zweiten Notfall. Profis nennen das Eigenschutz.'},
  {q:'Warum solltest du deine Adresse und Mamas Handynummer auswendig können?',options:['Für den Test in der Schule','Um im Notfall immer Hilfe rufen zu können','Für das Klassenbuch','Muss man nicht'],correct:1,img:'🏠',why:'Handy leer, verlaufen, Unfall – wer Adresse und Nummer im Kopf hat, ist nie wirklich verloren. Ein unsichtbares Sicherheitsnetz.'},
  {q:'„NUR HEUTE! Traktormodell 30 % billiger!" Was ist der Trick?',options:['Kein Trick, schnell kaufen','Zeitdruck soll dich am Nachdenken hindern','Der Traktor ist magisch','Morgen ist der Laden zu'],correct:1,img:'⏰',why:'„Nur heute" ist der älteste Verkäufertrick der Welt. Wer dich hetzt, will nicht, dass du vergleichst. Gute Angebote kommen wieder.'},
  {q:'Du verkaufst 3 Kisten Äpfel für je 4 €. Jede Kiste hat dich 2 € gekostet. Wie viel GEWINN machst du?',options:['12 €','6 €','4 €','2 €'],correct:1,img:'🍎',why:'Einnahmen: 3 mal 4 = 12 €. Kosten: 3 mal 2 = 6 €. Gewinn = 12 minus 6 = 6 €. Gewinn ist NIE das, was reinkommt – sondern das, was übrig bleibt!'},
  {q:'Dein Kumpel bietet: sein altes Fahrrad gegen deine Spielkonsole. Was ist schlau?',options:['Sofort ja sagen','Werte vergleichen und eine Nacht drüber schlafen','Sofort nein schreien','Die Konsole verstecken'],correct:1,img:'🚲',why:'Große Tauschgeschäfte nie sofort entscheiden! Was ist beides wirklich wert? Eine Nacht Schlaf hat schon viele schlechte Deals verhindert.'},
  {q:'Was kommt beim Acker ZUERST?',options:['Säen, dann pflügen','Pflügen, dann säen','Ernten, dann säen','Alles gleichzeitig'],correct:1,img:'🌱',why:'Erst den Boden lockern, dann die Saat rein – sonst liegt sie oben und die Vögel feiern ein Festmahl. Richtige Reihenfolge spart doppelte Arbeit.'},
  {q:'Deine Maschine macht plötzlich komische Geräusche. Was ist schlau?',options:['Weiterfahren, bis sie ganz kaputt ist','Anhalten und prüfen (lassen)','Schneller fahren','Radio lauter drehen'],correct:1,img:'🔧',why:'Früh entdeckt = kleine, billige Reparatur. Ignoriert = Riesenschaden. Die besten Landwirte HÖREN auf ihre Maschinen.'},
  {q:'Lego-Set für 60 €. Du bekommst 5 € Taschengeld pro Woche. Wie lange sparst du?',options:['6 Wochen','12 Wochen','20 Wochen','60 Wochen'],correct:1,img:'🧱',why:'60 geteilt durch 5 = 12 Wochen. Wer sein Ziel in Wochen umrechnen kann, weiß immer, wo er steht – das ist ein echter Sparplan!'},
  {q:'Dein Freund will deine Hausaufgaben abschreiben. Was ist schlau?',options:['Abschreiben lassen','Ihm ERKLÄREN, wie es geht','Auslachen','Wegrennen'],correct:1,img:'📚',why:'Beim Abschreiben lernt er nichts und ihr riskiert Ärger. Beim Erklären lernt ihr BEIDE – Erklären ist das beste Gehirntraining überhaupt.'},
  {q:'E-Mail: „Du hast ein iPhone gewonnen!! Klick hier!!" Was tust du?',options:['Schnell klicken','Nicht klicken und den Eltern zeigen','Adresse eingeben','An Freunde weiterleiten'],correct:1,img:'🎣',why:'Niemand verschenkt iPhones an Fremde. Solche Mails „angeln" nach deinen Daten – deshalb heißt der Trick Phishing. Wer nicht klickt, gewinnt wirklich.'},
  {q:'Die Werbung zeigt glückliche Bauern bei Sonnenuntergang auf dem Riesen-Traktor. Was wird hier verkauft?',options:['Der Sonnenuntergang','Ein GEFÜHL – damit du den Traktor toll findest','Die Mütze des Bauern','Nichts'],correct:1,img:'🌅',why:'Werbung verkauft fast immer Gefühle statt Fakten. Du sollst das schöne Gefühl mit dem Produkt verwechseln. Wer das durchschaut, kauft nur, was er braucht.'},
  {q:'Dein Freund fällt vom Baum und steht nicht auf. Was tust du?',options:['Ihn schnell hochziehen','NICHT bewegen – 112 und Erwachsene rufen, bei ihm bleiben, mit ihm reden','Weglaufen','Ihn schütteln'],correct:1,img:'🌳',why:'Bei schlimmen Stürzen kann Bewegen mehr kaputt machen. Dein Job: Hilfe rufen, dableiben, ruhig reden, warm halten. Genau so retten Kinder wirklich Leben.'},
  {q:'Der Nachbar zahlt 5 € fürs Rasenmähen (1 Stunde). Das Kino kostet 10 €. Wie oft musst du mähen?',options:['Einmal','Zweimal','Fünfmal','Zehnmal'],correct:1,img:'💶',why:'2 mal 5 € = 10 €. Wer Arbeit in Ziele umrechnet, versteht Geld wirklich. Und selbst verdientes Kino-Geld fühlt sich doppelt so gut an!'},
  {q:'Job A: 1 Stunde Rasenmähen für 5 €. Job B: 2 Stunden Blätterrechen für 8 €. Was ist PRO STUNDE besser?',options:['Job A (5 € pro Stunde)','Job B (4 € pro Stunde)','Beide gleich','Der mit weniger Arbeit'],correct:0,img:'⏱️',why:'A: 5 € pro Stunde. B: 8 geteilt durch 2 = 4 € pro Stunde. Nicht die Gesamtsumme zählt, sondern was deine STUNDE wert ist – so denken Profis.'},
  {q:'Warum legen schlaue Leute einen „Notgroschen" zur Seite, den sie NIE anfassen?',options:['Aus Langeweile','Für kaputte Dinge und Notfälle','Um anzugeben','Weil Münzen schön glänzen'],correct:1,img:'🪙',why:'Wenn der Reifen platzt oder das Handy kaputtgeht, rettet dich der Notgroschen – ohne Schulden, ohne Panik. Dein persönlicher Schutzschild.'},
  {q:'„ALLE in der Klasse haben das neue Handy!" Was fragst du dich?',options:['Wo ist meins?!','Brauche ICH es wirklich – und was würde ich dafür aufgeben?','Wer hat das schönste?','Nichts'],correct:1,img:'📱',why:'„Alle haben es" ist kein Grund – das ist Gruppendruck. Schlaue Käufer fragen: Brauche ich das? Was könnte ich mit dem Geld sonst machen?'},
  {q:'Ein fast neues Fahrrad gibt es gebraucht für die Hälfte. Was ist schlau?',options:['Gebraucht ist immer schlecht','Prüfen, Probefahren – gebraucht kaufen kann richtig clever sein','Nur neu kaufen','Gar kein Fahrrad'],correct:1,img:'♻️',why:'Viele Dinge verlieren beim ersten Besitzer die Hälfte an Wert, sind aber fast wie neu. Prüfen, handeln, sparen – oft der klügste Deal überhaupt.'}
];
